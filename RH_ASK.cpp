...
void RH_ASK::timerSetup() {
  ...
  // TCA1.SINGLE.EVCTRL &= ~(TCA_SINGLE_CNTEI_bm);

  // /* set the period */
  // // The prescaler is set to 1

  // const int period = round(F_CPU / 1.0 / 8.0 / _speed);
  // TCA1.SINGLE.PER = period;
  // Serial.println(period);

  // TCA0.SINGLE.CTRLA = TCA_SINGLE_ENABLE_bm /* start timer */ | TCA_SINGLE_CLKSEL_DIV1_gc;
  ...
  // /* set Normal mode */
  // //TCB2.CTRLB = TCB_WGMODE_NORMAL_gc;
  ...
  // // TCB2.CTRLA = TCB_ENABLE_bm /* start timer */ | TCB_CLKSEL_CLKDIV1_gc;
  ...
  // TCB2.EVCTRL = TCB_CAPTEI_bm | TCB_FILTER_bm;
  ...
  if (ITimer1.attachInterrupt(_speed * 8, megaAVR_IRS))
...
bool RH_ASK::available() {
  if (_mode == RHModeTx)
    return false;
  setModeRx();
  // Serial.print("_rxBufFull: ");
  // Serial.println(_rxBufFull ? "true" : "false");
  if (_rxBufFull) {
    validateRxBuf();
    _rxBufFull = false;
  }
...
//   //TCB2.INTFLAGS = 0; /* Clear the interrupt flag */
//   Serial.print(TCB2.INTFLAGS);
//   Serial.print(" ");
//   Serial.print(TCB2.CCMP);
//   Serial.print(" ");
//   Serial.println(TCB2.CNT);
...
void RH_ASK::receiveTimer() {
  ...
  if (_rxPllRamp >= RH_ASK_RX_RAMP_LEN) {
    ...
    // Not in a message, see if we have a start symbol
      ...
      //Serial.println("Got the start symbol");
      _rxActive = true;
      _rxBitCount = 0;
      _rxBufLen = 0;
    }
