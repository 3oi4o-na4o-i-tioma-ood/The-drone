...
lora_init(void)
{
   ...
   printf("spi_bus_initialize returned %d\n", ret);
   assert(ret == ESP_OK);
...
lora_send_packet(uint8_t *buf, int size)
{
   ...
   printf("Wrote REG_FIFO_ADDR_PTR\n");

   for(int i=0; i<size; i++) 
      lora_write_reg(REG_FIFO, *buf++);
   printf("Wrote REG_FIFO\n");
   lora_write_reg(REG_PAYLOAD_LENGTH, size);
   printf("Wrote REG_PAYLOAD_LENGTH\n");
   /*
    * Start transmission and wait for conclusion.
    */
   lora_write_reg(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_TX);
   printf("Wrote REG_OP_MODE\n");
   while((lora_read_reg(REG_IRQ_FLAGS) & IRQ_TX_DONE_MASK) == 0)
      vTaskDelay(2);

   printf("Read REG_IRQ_FLAGS\n");

   lora_write_reg(REG_IRQ_FLAGS, IRQ_TX_DONE_MASK);
   printf("Wrote REG_IRQ_FLAGS\n");
}
